subroutine reademissions_feux(nemission,outlon0,outlat0,&
     ...
     emifeux,index_feux,nbtime,date_flex,frp,h)
  ...
  real emifeux(nxmax,nymax,nbtime),frp(nxmax,nymax,nbtime),h(nxmax,nymax,nbtime)
  integer nval(nxmax,nymax,nbtime)
  ...
  integer ufile_cont,cont_varid,time_varid,frp_varid,ale_varid

  real,allocatable :: lon_nc(:),lat_nc(:)
  integer,allocatable :: indlon_nc(:),indlat_nc(:)
  real,allocatable :: emis_nc(:,:,:),frp_nc(:,:,:),h_nc(:,:,:)
...
!  write(*,*) trim(pref),' >check date : dim =',nbtime_nc
  ...
  allocate(frp_nc(nblon_nc,nblat_nc,nbtime_nc))
  allocate(h_nc(nblon_nc,nblat_nc,nbtime_nc))

  call check(nf_inq_varid(ufile_nc,'longitude',lon_varid))
  call check(nf_get_var_real(ufile_nc,lon_varid,lon_nc))
  call check(nf_inq_varid(ufile_nc,'latitude',lat_varid))
  call check(nf_get_var_real(ufile_nc,lat_varid,lat_nc))
  call check(nf_inq_varid(ufile_nc,'time',time_varid))
  call check(nf_get_var_int(ufile_nc,time_varid,time_nc))
  !write(*,*) trim(pref),' >get var : OK'

  if(trim(lab_feuxbiomasse).eq.'gfas1.0')then
  ...
  elseif(trim(lab_feuxbiomasse).eq.'gfas0.1')then
     call check(nf_inq_varid(ufile_nc,'cofire',emis_varid))
     call check(nf_inq_varid(ufile_nc,'frpfire',frp_varid))
     call check(nf_inq_varid(ufile_nc,'apt',ale_varid)) !altitude of plume top
  ...
  if (trim(lab_feuxbiomasse).eq.'gfas0.1') then
   call check(nf_get_var_real(ufile_nc,frp_varid,frp_nc))
   call check(nf_get_var_real(ufile_nc,ale_varid,h_nc))
  endif
  ...
  if(trim(lab_feuxbiomasse).eq.'gfas0.1')then
     ...
     !frp_nc=0.00166347774627537*frp_nc+54.5055118344587
     !h_nc=0.09842236949323242*h_nc+3224.95078815253
     !h_nc=0.148017029588146*h_nc+4849.92599148521
     do ix=1,nblon_nc
        do jy=1,nblat_nc
           do n=1,nbtime_nc              
              if(emis_nc(ix,jy,n) .lt. 1e-20) then
               emis_nc(ix,jy,n)=0.
              endif
              if(frp_nc(ix,jy,n) .lt. 0) then
               frp_nc(ix,jy,n)=0.
              endif
              if(h_nc(ix,jy,n) .lt. 10) then
               h_nc(ix,jy,n)=0.
              endif
           enddo
        enddo
  ...
  if(trim(lab_feuxbiomasse).eq.'gfas0.1')then  
   write(*,*) trim(pref),' >get FRP : OK'
   write(*,*) trim(pref),' >range:',minval(frp_nc),maxval(frp_nc)
   write(*,*) trim(pref),' >get top altidude : OK'
   write(*,*) trim(pref),' >range:',minval(h_nc),maxval(h_nc)
  endif
...
!*********************************************
  ...
  emifeux=0.
  index_feux=0
  !frp is the sum off frpi*dSi over FLEXPART grid result in W
  frp=0.
  h=0.
  nval=0
...
!c Now, we know that emis_nc(ix,jy) goes inside emissions(indx,indy)
        do it=1,nbtime
           emifeux(indx,indy,it)=emifeux(indx,indy,it)+&
                ...
                area_nc(ix,jy)/area(indx,indy)
           frp(indx,indy,it)=frp(indx,indy,it)+&
                frp_nc(ix,jy,indtime(it))*&
                area_nc(ix,jy)
           h(indx,indy,it)=max(h(indx,indy,it),&
                h_nc(ix,jy,indtime(it)))
           !nval(indx,indy,it)=nval(indx,indy,it)+1
        enddo
  ...
  do ix=1,numxgrid
     do jy=1,numygrid
        do it=1,nbtime
           ...
           n=n+1
           !if (nval(ix,jy,it).gt.0)h(ix,jy,it)=h(ix,jy,it)/nval(ix,jy,it)
        enddo
     enddo
  enddo
  maxemis=maxval(emifeux)
  write(*,*)trim(pref),'max  emifeux: ',maxemis
  write(*,*)trim(pref),'mean emifeux: ',meanemis/n
  write(*,*)trim(pref),'min frp:',minval(frp),'max frp:',maxval(frp)
  write(*,*)trim(pref),'min h:',minval(h),'max h:',maxval(h)
