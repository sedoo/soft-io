program flexpartplot_latlon
  ...
  real,allocatable :: emissions(:,:,:),emifeux(:,:,:)
  real frp
...
98      format(i8,2f9.4,4f8.1,f8.2,4f8.1,3f6.1,5(2f8.3,f7.0,f6.1,f8.1))
                    ...
                    do kz=1,numzgrid-1 ! then, all levels                               
                       ...
                       if(anth1_both2_bbur3.ge.2 .and. contribfire.gt.0.)then
                          ...
                          if (abs(ym).ge.60.) then
                             ...
                             frp=emifeux(ix,jy,it)*area(ix,jy)*24.*3600./(106.*1.55) !(GJ.day-1)
                             if (frp.gt.1e4) then
                                if (frp.gt.1e5) then
     ...
     if(anth1_both2_bbur3.ge.2)then
        ...
        open(210,file=trim(outdir)//trim(flight)//'co_'//trim(firestr)//'_prm.txt')
        write(210,*) 0.
        write(210,*) 0.
        write(*,*) trim(pref),trim(outdir)//trim(flight)//'co_'//trim(firestr)//'_prm.txt'
